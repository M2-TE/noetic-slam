...
public:
    TSDF_Map() {
        ...
        dagLeafLevel.leaves.emplace_back();
        for (uint i = 0; i < dagLevelCount - 2; i++) {
...
private:
    inline void insert_point(Eigen::Vector3f coordPos) {
        // convert from real coordinate to voxel
        Eigen::Vector3i voxelPos = (coordPos * coordToVoxelRatio).cast<int>();
        ...
        // check if root is already present, create it if not
        auto result = dagRootLevel.hashmap.try_emplace(rootPos);
        if (result.second) {
            // set up pointer to root and then create it
